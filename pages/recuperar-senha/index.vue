...
async function callBeforeMount() {
...
async function handleEmailFormSubmit() {
  ...
  if (status.value.ok) {
    ...
    emailModal.value.hide();
  } else {
    showToast("O email não pode ser enviado", status.value.message, "warn");
  }
}
...
watch([isLoaded, isMounted], ([newValueIsLoaded, newValueIsMounted]) => {
  if (newValueIsLoaded && newValueIsMounted) {
